# YouTube Player - Microsoft Teams App (Adaptive Cards)

This sample is a Microsoft Teams application which explores the basics of task modules in Microsoft Teams and how to collect input from users in a custom Teams tab. An Adaptive Card that accepts the ID of a video on YouTube. When the task module is invoked, it will display the video using the YouTube embedded player.
...
3. Create a new *Web App* with Windows App Service Plan and give it the name of your tab, the same you used when asked for URL in the Yeoman generator. In your case https://youtubeplayer.azurewebsites.net.
...
11. Run the following command to set up the remote repository: `git remote add azure https://<username>@youtubeplayer.scm.azurewebsites.net:443/youtubeplayer.git`. You need to replace <username> with the username of the App Credentials you retrieved in _Deployment Credentials_. You can also copy the URL from *Options* in the Azure Web App.
...
13. Wait until the deployment is completed and navigate to https://youtubeplayer.azurewebsites.net/privacy.html to test that the web application is running
14. Done
